...
class DWPoseModel:
    ...
    def __init__(self, **kwargs):
        ...
        self.return_openpose = kwargs.get("return_openpose", True)

    def preprocess(self, img, out_bbox, input_size):
    ...
    def postprocess(self, outputs: List[np.ndarray],
        ...
        keypoints, scores = np.array(all_key), np.array(all_score)
    ...
    def predict(self, *data):
        ...
        resized_img, center, scale = self.preprocess(oriImg, bboxs, model_input_size)
        outputs = self.inference(resized_img)
        # postprocess for rtm-pose model output.
        keypoints = self.postprocess(outputs, model_input_size, center, scale)
